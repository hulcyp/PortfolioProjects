...
namespace Monky
{
	...
	void P3DImporter::importMaterials()
	{
		...
		for( int i = 0; i < numMats; ++i )
		{
			...
			Material* material = Material::createOrGetMaterial( materialNameStr, "DiffuseShader" );
		...
		for( int i = 0; i < numMeshes; ++i )
		{
			...
			{					
				...
				else
				{
					...
					if( meshType == 1 )
					{
						Material::getMaterial( matName )->changeShaderProgram( "SkinningShader" );
					}
				}
	...
	{
		Texture* tex = Texture::CreateOrGetTexture( texFile, texFile );
