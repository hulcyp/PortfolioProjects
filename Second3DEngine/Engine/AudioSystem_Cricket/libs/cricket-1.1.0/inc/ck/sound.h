...
public:
    ...
    /** Sets the effect bus to which this sound's audio is sent to be processed, or
     ...
     @par Android note: 
     Effects cannot be applied to streams in formats other than .cks. */
    void setEffectBus(CkEffectBus*);

    /** Gets the effect bus to which this sound's audio is sent to be processed, or
      NULL for dry output (no effects). */
    CkEffectBus* getEffectBus();
    ...
    /** Sets a callback to be called for each slice of audio coming from
     ...
     @par Android note: 
     This does not work for streams in formats other than .cks. 
     @deprecated Instead, use setEffectBus to send this sound to an effect bus containing a custom effect. */
    ...
    /** Creates a custom stream.
      ...
      Note that the CkCustomStream object will be deleted when the CkSound object is destroyed. */
