...
namespace pdh
{
	...
	XMLParser::XMLParser( const char* fileLocation )
		: m_fileLocation( fileLocation )
	{
		int errorID = m_document.LoadFile( m_fileLocation.c_str() ); 
		if(  errorID != tinyxml2::XML_NO_ERROR )
		{
	...
	void XMLParser::outputXMLErrorMsgToWindow( const char* format, ... )
	{
		...
		bool response = displayYesNoMessage( ms_ErrorTitle, msg.c_str() );
	...
	void XMLParser::outputXMLErrorMsgToConsole( const char* format, ... )
	{
		...
		printErrorToOutputConsole( msg.c_str() );
	}
