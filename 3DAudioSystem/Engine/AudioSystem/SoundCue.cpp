...
namespace Monky
{
	...
	{
		dsp->initialize();
		m_soundCueImpl->pushDSP( dsp->m_dspImpl );
	}
	//----------------------------------------------
	void SoundCue::popDSP()
	{
		m_soundCueImpl->popDSP();
	}
	//----------------------------------------------
	...
	{
		XMLNode* dspNode;
		std::vector< std::pair< Monky_DSP_Type, XMLNode* > > dspsToAdd;
		for( dspNode = soundCueNode->FirstChildElement( "DSP" ); dspNode != nullptr; dspNode = dspNode->NextSiblingElement( "DSP" ) )
		{
			std::string strType = parser.getXMLAttributeAsString( dspNode, "type", "" );
			Monky_DSP_Type type = DSP::getDSPTypeFromString( strType );
			if( type != MONKY_DSP_TYPE_FORCEINT )
			{
				dspsToAdd.push_back( std::pair< Monky_DSP_Type, XMLNode* >( type, dspNode ) );
			}
		}
		for( int i = (int)dspsToAdd.size() - 1; i >= 0 ; --i )
		{
			DSP* dsp = DSP::createDSPFromXMLNodeBasedOnType( dspsToAdd[i].first, parser, dspsToAdd[i].second );
			if( dsp != nullptr )
				soundCue->pushDSP( dsp );
		}
	}
	//----------------------------------------------
