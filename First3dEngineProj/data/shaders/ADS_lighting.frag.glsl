...
float FOG_SCALE = 100.0;
vec4 FOG_COLOR = vec4( 0.2, 0.2, 0.2, 1.0 );
...
vec4 getDirectLight( in Light light, in float attenuationFactor, in vec3 normal, in vec3 lightDirFromVertexTBNNormalized, in vec3 lightDirTBNNormalized, in vec2 texCoord )
{
	...
	return vec4( color.rgb * texture( uDiffuseMap, texCoord ).rgb * directIntensity * light.color.rgb, color.a );
}
...
vec4 getColorFromLighting( in vec2 texCoord )
{
	...
	if( unlit != 0 )
	{
		tempFragColor = color * texture( uDiffuseMap, texCoord );
	}
	else
	{			
...
{	
	...
	if( enableParallax != 0 )
	{
		texCoord = texCoord0 + texture( uSpecularMap, texCoord0 ).r * 0.03 * ( -cameraToVertex.xy );
	}

	fragColor = getColorFromLighting( texCoord );
}
