...
namespace pdh
{
	...
	{
		return Vector2<T>( clamp( x, floor, ceil ), clamp( y, floor, ceil ) );
	}
	template< typename T >
	Vector2<T> Vector2<T>::clamp( const Vector2<T>& floor, const Vector2<T>& ceil ) const
	{
		return Vector2<T>( clamp( x, floor.x, ceil.x ), clamp( y, floor.y, ceil.y ) );
	}
	template< typename T >
	...
	template< typename T >
	Vector2<T> parseStringToVec2( const std::string& str )
	{
		std::vector< std::string > tokens;
		stringTokenizer( str, tokens, "," );
		Vector2<T> temp;
