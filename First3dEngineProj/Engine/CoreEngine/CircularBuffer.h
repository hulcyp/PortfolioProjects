...
namespace pdh
{
	...
	T& CircularBuffer<T>::operator[]( int index )
	{
		...
		size_t relIndex = wrap( m_frontIndex - index, 0, (int)m_buffer.size() - 1 );
	...
	void CircularBuffer<T>::pushFront( const T& element )
	{
		...
		if( m_bufferFull )
		{
			newIndex = wrap( m_frontIndex + 1, 0, (int)m_buffer.size() - 1 );
			backIndex = wrap( backIndex + 1, 0, (int)m_buffer.size() - 1 );
