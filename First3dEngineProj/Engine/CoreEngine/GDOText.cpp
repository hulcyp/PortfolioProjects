...
namespace pdh
{
	GDOText::GDOText( Actor* actor, const std::string& type, float currentTime, Color4f color, float fontHeight )
		:	GraphicDebugObject( actor->getPosition(), mat4f::identity(), currentTime )
		,	m_actor( actor )
		,	m_type( type )
		,	m_color( color )
	...
	{
		Material* material = renderer->getMaterial( "DebugMaterial" );
		Font* font = renderer->getFont( "mainFont_72" );
