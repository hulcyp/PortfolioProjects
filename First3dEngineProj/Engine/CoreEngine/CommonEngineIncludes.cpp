...
namespace pdh
{
	...
	ConsoleSystem * initConsole( int width, int height, int bufferSize, Renderer* renderer )
	{
		if( !g_ConsoleSystem )
		{
			...
			g_ConsoleSystem->registerCommand( "tagactor", createActorGDOCmd, "Used to tag an actor for debug" );
			g_ConsoleSystem->registerCommand( "listactors", listActorsCmd, "Lists all actors currently in the level" );
			g_ConsoleSystem->registerCommand( "clearGDOs", clearAllGDOsCmd, "Clears all active GDOs" );
			g_ConsoleSystem->registerCommand( "tagactortext", createTextGDOCmd, "Tag an actor to display text info" );
	...
	void createOrientationGDO( float size, const vec3f& pos, const mat4f& rotation, float showUntilTime )
	{
